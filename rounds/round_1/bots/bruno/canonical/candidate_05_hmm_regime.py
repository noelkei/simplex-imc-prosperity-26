...
candidate_05_hmm_regime:

Base: candidate_04_kalman_imb (teammate + Kalman FV, ~10,090+ P&L)
Added: 2-state Hidden Markov Model for regime detection + adaptive HS

WHY HMM:
  ACO autocorr profile (lag-50 = 0.717) shows the process has PERSISTENT
  deviations that are far stronger than AR(1) predicts. This implies distinct
  regimes: CALM periods where price stays near FV, and VOLATILE periods where
  price sustains large deviations for 100+ ticks.

  In CALM regime: tight spread is safe → more fills, higher P&L.
  In VOLATILE regime: wide spread needed → avoid adverse selection.

HMM design:
  State 0 (CALM):    σ_obs = 2.0  (small mid-price changes)
  State 1 (VOLATILE): σ_obs = 5.0  (large mid-price changes)

  Transition matrix (persistent regimes, calibrated from EDA autocorr):
    P(CALM→CALM)    = 0.97   (calm lasts ~33 ticks on average)
    P(VOL→VOL)      = 0.93   (volatile lasts ~14 ticks on average)

  Online update: forward algorithm each tick using |Δmid| as observation.
  Since we only care about P(VOL), we track p_vol ∈ [0, 1].

Adaptive parameters based on P(VOLATILE):
  HS      = round(3 + 3 * p_vol)   → HS = 3 (calm) to 6 (volatile)
  buy_sz  = round(72 - 32 * p_vol) → 72 (calm) to 40 (volatile)
  sell_sz = round(72 - 32 * p_vol) → same

  At neutral position (p_vol=0): HS=3, very tight → many fills.
  At fully volatile (p_vol=1): HS=6, wide → safe quoting.

Combined with Kalman FV: quotes adapt to BOTH the estimated fair value
AND the current volatility regime — two independent sources of edge.
"""
...
HMM_SIGMA_CALM = 2.0
HMM_SIGMA_VOL  = 5.0
HMM_P00 = 0.97   # P(CALM → CALM)
HMM_P11 = 0.93   # P(VOL  → VOL)
...
class Trader:
    ...
    # ── HMM forward update ────────────────────────────────────────────────────
    # Observation: |delta_mid| (absolute mid-price change since last tick)
    # Unnormalized Gaussian emission — normalization step cancels the constant.

    def _gauss(self, x, sigma):
        return math.exp(-0.5 * (x / sigma) ** 2)

    def _hmm_update(self, sd, delta_mid):
        p_vol = sd.get("hmm_pv", 0.5)   # P(VOLATILE)
        p_calm = 1.0 - p_vol
        ...
        sd["hmm_pv"] = new_vol
        return new_vol   # return P(VOLATILE)
    ...
    # ── ACO: Kalman FV + HMM regime + imbalance + centered skew ──────────────
    ...
    def _trade_aco(self, state, sd):
        ...
        delta    = mid - prev_mid
        sd["prev_mid"] = mid
        p_vol = self._hmm_update(sd, delta)

        # Adaptive half-spread and quote sizes
        hs      = round(HS_MIN + (HS_MAX - HS_MIN) * p_vol)   # 3→6
        buy_sz  = round(72 - 32 * p_vol)                       # 72→40
        sell_sz = buy_sz

        # ── One-sided book: only bids ──
        ...
        # ── Passive quotes: Kalman FV + imbalance + HMM-adaptive HS ──
