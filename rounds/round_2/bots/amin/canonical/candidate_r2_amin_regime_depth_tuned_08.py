...
IPR_RICH_TRIM_EDGE = 9
IPR_REPOST_MAX_EXTRA = 3
IPR_PRESSURE_REPOST_COEF = 3.5
MARKET_ACCESS_BID = 9
...
class Trader:
    ...
    def _trade_ipr(self, state: TradingState, sd: dict):
        ...
        if best_bid is not None and self._buy_cap(limit) > 0:
            ...
            depth_shift = self._depth_price_shift(od)
            pressure_score = max(0.0, 1.05 * imbalance + 0.15 * depth_shift)
            ...
            if pressure_score < 0.35:
                desired_repost = min(desired_repost, 18)
            elif pressure_score < 0.75:
                desired_repost = min(desired_repost, 32)
            if pos + self._buy_used >= aggressive_limit:
                desired_repost = min(desired_repost, 10)
            self._add_buy(orders, IPR, bid_px, desired_repost, limit)

        effective_pos = pos + self._buy_used - self._sell_used
        if effective_pos > 66 and best_bid is not None and visible_mid is not None and best_bid >= visible_mid + IPR_RICH_TRIM_EDGE:
            self._add_sell(orders, IPR, best_bid, min(10, effective_pos - 66), limit)
    ...
    def _trade_aco(self, state: TradingState, sd: dict):
        ...
        tight_regime = spread is not None and spread <= 14
        wide_regime = spread is not None and spread >= 18
        active_regime = bool(tight_regime or wide_regime)
        if tight_regime:
            predictive_shift = 0.52 * micro_shift + 0.24 * depth_shift + 1.1 * imbalance_shift
        elif wide_regime:
            predictive_shift = 0.34 * micro_shift + 0.18 * depth_shift + 0.9 * imbalance_shift
        else:
            predictive_shift = 0.18 * micro_shift + 0.02 * depth_shift + 0.45 * imbalance_shift
        ...
        if tight_regime:
            dynamic_take_edge -= 0.25
        elif wide_regime:
            dynamic_take_edge += 0.15
        else:
            dynamic_take_edge += 1.0
        if od.sell_orders:
            for ask in sorted(od.sell_orders):
                if self._buy_cap(limit) <= 0:
                    break
        ...
        buy_size = 46 if tight_regime else 34 if wide_regime else 14
        sell_size = 46 if tight_regime else 34 if wide_regime else 14
