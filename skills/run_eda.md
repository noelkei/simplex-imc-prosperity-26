...
24. Decide explicitly whether the work needs only raw-data EDA or also a retrospective run-informed EDA addendum. If meaningful run evidence changes product roles, linked-product framing, family exposure, churn, or hold horizon, create the addendum instead of burying that learning in testing only.
25. Produce reproducible output: command notes, script, table, plot, or optional notebook. Use a notebook when exploratory code, charts, or feature experiments would help a human inspect or extend the work; keep Markdown as the canonical handoff.
26. Write EDA summaries under `../rounds/round_X/workspace/01_eda/` using `eda_<short_question>.md` and the EDA summary template. Use `../docs/templates/retrospective_eda_addendum_template.md` when run-informed EDA is needed.
27. Include `Product Scope`, `Algorithmic vs Manual Scope`, `Round Adaptation Check`, `Prior-Round Compatibility` when used, `Artifact Index`, `Data Quality And Filters`, `Feature Inventory`, `Product / Role Classification`, `Feature Engineering Notes`, `Feature Promotion Decisions`, `Multivariate Feature Map`, `Redundancy / Dimensionality Check`, `Cross-Product Relationships`, `Retrospective EDA Decision`, `Process / Distribution Hypotheses`, `Multivariate Model Notes`, `Conditional Patterns / Regimes`, `Linked-Product / Book Framing Notes`, `Threshold / Execution Findings`, `Signal Hypotheses`, `Downstream Feature Contract Implications`, `Negative Evidence`, and `Downstream Use / Agent Notes`.
...
41. Handoff with method, filters, linked artifacts, findings, feature origin/usability/role/status, evidence limits, negative evidence, carry-forward principles if any, untested hypotheses if any, assumptions, what to use, what not to trust yet, validation needed, and next action.
