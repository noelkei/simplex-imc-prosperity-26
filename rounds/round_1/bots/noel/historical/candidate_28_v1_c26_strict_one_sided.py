...
class Trader:
    ...
    def _trade_aco_strict_one_sided(self, state):
        ...
        if best_bid is not None and best_ask is None:
            ...
            if pos > 20 and best_bid >= ACO_FV - 3:
                virtual_pos = self._add_sell(orders, ACO, best_bid, min(visible, max(14, min(pos, 32))), virtual_pos, max_pos)
            elif pos > 0 and best_bid >= ACO_FV - 1:
                virtual_pos = self._add_sell(orders, ACO, best_bid, min(visible, max(8, pos)), virtual_pos, max_pos)
            elif best_bid > ACO_FV + 7 and pos > -24:
                virtual_pos = self._add_sell(orders, ACO, best_bid, min(visible, 12), virtual_pos, max_pos)
            inv_skew = round((pos / max_pos) * 3)
            if pos > 8 or best_bid >= ACO_FV + 4:
                ask_px = max(best_bid + 1, ACO_FV + 6 - inv_skew)
                ask_size = 36 if pos > 20 else 14 if pos >= 0 else 8
        ...
        if best_ask is not None and best_bid is None:
            ...
            if pos < -20 and best_ask <= ACO_FV + 3:
                virtual_pos = self._add_buy(orders, ACO, best_ask, min(visible, max(14, min(-pos, 32))), virtual_pos, max_pos)
            elif pos < 0 and best_ask <= ACO_FV + 1:
                virtual_pos = self._add_buy(orders, ACO, best_ask, min(visible, max(8, -pos)), virtual_pos, max_pos)
            elif best_ask < ACO_FV - 7 and pos < 24:
                virtual_pos = self._add_buy(orders, ACO, best_ask, min(visible, 12), virtual_pos, max_pos)
            inv_skew = round((pos / max_pos) * 3)
            if pos < -8 or best_ask <= ACO_FV - 4:
                bid_px = min(best_ask - 1, ACO_FV - 6 - inv_skew)
                bid_size = 36 if pos < -20 else 14 if pos <= 0 else 8
    ...
    def run(self, state: TradingState):
        ...
        try:
            ...
            result[IPR] = []
        try:
            result[ACO] = self._trade_aco_strict_one_sided(state)
        except Exception:
            result[ACO] = []
