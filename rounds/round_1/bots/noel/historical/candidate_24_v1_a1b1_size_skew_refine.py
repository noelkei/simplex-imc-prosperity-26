...
class Trader:
    ...
    def _aco_cross_buy_qty(self, available, edge, pos):
        qty = available
        if pos > 50 and edge < 6:
            qty = min(qty, 14)
        elif pos > 30 and edge < 4:
            qty = min(qty, 24)
        elif pos > 15 and edge < 2:
            qty = min(qty, 36)
        return qty

    def _aco_cross_sell_qty(self, available, edge, pos):
        qty = available
        if pos < -50 and edge < 6:
            qty = min(qty, 14)
        elif pos < -30 and edge < 4:
    ...
    def _passive_size(self, pos, side):
        if side == "buy":
            if pos < -30:
                return 80
            if pos > 50:
    ...
    def _trade_aco_size_skew(self, state):
        ...
        for ask in sorted(od.sell_orders):
            ...
            available = -od.sell_orders[ask]
            edge = ACO_FV - ask
            qty = self._aco_cross_buy_qty(available, edge, pos)
        ...
        for bid in sorted(od.buy_orders, reverse=True):
            ...
            edge = bid - ACO_FV
            qty = self._aco_cross_sell_qty(available, edge, pos)
        ...
        if best_bid is not None and best_ask is not None:
            ...
            bid_width = 5 + max(0, pos) // 28
            ask_width = 5 + max(0, -pos) // 28
            if pos < -25:
                bid_width = max(3, bid_width - 1)
            if pos > 25:
                ask_width = max(3, ask_width - 1)

            bid_px = min(best_bid + 1, ACO_FV - bid_width - inv_skew)
            ask_px = max(best_ask - 1, ACO_FV + ask_width - inv_skew)
            if bid_px < best_ask:
                virtual_pos = self._add_buy(
                    orders, ACO, bid_px, self._passive_size(pos, "buy"), virtual_pos, max_pos
                )
            if ask_px > best_bid:
                virtual_pos = self._add_sell(
                    orders, ACO, ask_px, self._passive_size(pos, "sell"), virtual_pos, max_pos
                )
        return orders
    ...
    def run(self, state: TradingState):
        ...
        try:
            ...
            result[IPR] = []
        try:
            result[ACO] = self._trade_aco_size_skew(state)
        except Exception:
            result[ACO] = []
