...
# All OTM strikes whose market mid is far above (intrinsic + small premium)
STRIKES = {
    "VEV_5300": (5300, 6.0),
    "VEV_5400": (5400, 4.0),
    "VEV_5500": (5500, 2.5),
...
class Trader:
    def run(self, state: TradingState):
        ...
        for symbol, (strike, premium) in STRIKES.items():
            out = self._trade_strike(state, symbol, strike, premium, vex_mid, vex_move)
            if out:
                orders[symbol] = out
    ...
    def _trade_strike(self, state: TradingState, symbol: str, strike: int, premium: float, vex_mid: float, vex_move: float) -> List[Order]:
        ...
        # Don't trade if fair is too close to mid (no edge)
