...
class Trader:
    def run(self, state: TradingState):
        ...
        late_freeze = session_progress(state.timestamp) >= LATE_FREEZE_PROGRESS
    ...
    def _trade_strike(
        ...
        can_add_buy = not late_freeze
        can_add_sell = not late_freeze
        can_reduce_buy = position < 0
        can_reduce_sell = position > 0

        if buy_cap > 0 and ask <= fair - edge:
            qty = min(clip, buy_cap, max(0, -depth.sell_orders.get(ask, 0)))
            if qty > 0 and (can_add_buy or can_reduce_buy):
                if late_freeze and can_reduce_buy:
        ...
        if sell_cap > 0 and bid >= fair + edge:
            ...
            if qty > 0 and (can_add_sell or can_reduce_sell):
                if late_freeze and can_reduce_sell:
                    qty = min(qty, abs(position))
                if qty > 0:
                    out.append(Order(symbol, bid, -qty))

        if not late_freeze and spread_value <= 6:
            if buy_cap > 0:
