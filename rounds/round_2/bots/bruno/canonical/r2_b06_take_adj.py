...
r2_b06_take_adj — Imbalance-adjusted take threshold (R1 Kalman)

HYPOTHESIS: The take loop currently uses raw fv_int as the threshold — it only
takes asks below 10000 and bids above 10000. But when imbalance predicts mid will
move to 10004 (imb=0.6, gain=4), asking at 10003 is actually CHEAP relative to
the adjusted FV, and we should take it.

This bot uses qfair (FV + imbalance_shift) as the take threshold, capped at
fv_int ± MAX_IMB_TAKE. This allows taking asks up to 4 ticks above raw FV when
signal is strongly bullish, or selling bids down to 4 ticks below raw FV when
signal is strongly bearish.

Expected P&L from adjustment (at n≈30k ticks per day, ~40% strong imbalance):
  - ~12k ticks with |imb|>0.5 → take threshold expands by 1-4 ticks on each side
  - New takes: asks at fv+1 to fv+3 when imb=0.7+, representing 0.5-2.5 tick edge
  - Each new take: ~1-2 tick P&L vs baseline (expected next-mid is above take price)

Parameters:
  KF_Q=0.005, KF_R=25.0 (stable R1 Kalman — isolates take-threshold effect)
  IMB_GAIN=4.0, IMB_CLIP=4.0
...
TAKE_POS_THRESH  = 60
TAKE_EDGE_THRESH = 2
MAX_IMB_TAKE     = 4   # max ticks above/below raw FV for imb-adjusted takes
...
class Trader:
    ...
    def _trade_aco(self, state, sd):
        ...
        # Two-sided: compute imbalance-adjusted fair value and take thresholds
        imb      = self._imbalance(od)
        micro    = self._clip(IMB_GAIN * imb, -IMB_CLIP, IMB_CLIP)
        qfair    = fv + micro
        inv_skew = round((pos / max_pos) * ACO_SKEW)

        # Imbalance-adjusted take thresholds, capped to ±MAX_IMB_TAKE from raw FV
        take_buy_thr  = min(round(qfair), fv_int + MAX_IMB_TAKE)
        take_sell_thr = max(round(qfair), fv_int - MAX_IMB_TAKE)

        # Takes: buy below adjusted threshold (includes asks slightly above raw FV when bullish)
        for ask in sorted(od.sell_orders):
            if ask >= take_buy_thr:
                break
        ...
        # Takes: sell above adjusted threshold (includes bids slightly below raw FV when bearish)
