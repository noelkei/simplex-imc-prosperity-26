...
13. Choose targeted checks from the EDA methodology: descriptive stats, distributions, volatility/regimes, spreads/microstructure, correlations, lead-lag, price-vs-trade alignment, volume behavior, or order book dynamics. Use the research environment when it improves the check: `pandas`/`numpy` for core tables, `polars` for large logs, `scipy`/`statsmodels`/`pingouin` for confidence and tests, `arch`/`ruptures` for decision-relevant regimes, `sklearn` for lightweight grouping or baselines, and `numba` for repeated loops.
