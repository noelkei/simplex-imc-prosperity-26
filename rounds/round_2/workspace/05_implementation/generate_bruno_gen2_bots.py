...
OUT = ROOT / "rounds" / "round_2" / "bots" / "bruno" / "canonical"
SPEC = "rounds/round_2/workspace/04_strategy_specs/spec_r2_gen2_bruno_battery.md"


COMMON = r'''from datamodel import Order, TradingState
import json
...
def cfg(bot_id: str, role: str, modes: list[str], **overrides):
    base = {
        "bot_id": bot_id,
        "role": role,
        "modes": modes,
        ...
        "aco_quote_offset": 5,
        "aco_half_spread": 5,
        "aco_inventory_skew": 3,
        "aco_aggression": 0.58,
        "aco_probe_take_edge": 5.0,
...
CANDIDATES = [
    (
        ...
        cfg("R2-G2-01-IPR-EXTREME-RERUN-CHAMPION", "ipr_extreme_rerun", ["ipr_extreme"]),
    ),
    (
        "candidate_r2_g2_02_ipr_drift_rerun_code_equiv.py",
        cfg("R2-G2-02-IPR-DRIFT-RERUN-CODE-EQUIV", "ipr_drift_rerun", ["ipr_drift"], ipr_clip=8, ipr_max_spread=4),
    ),
    (
        "candidate_r2_g2_03_ipr_extreme_flatter_inventory.py",
        cfg(
            "R2-G2-03-IPR-EXTREME-FLATTER-INVENTORY",
            "ipr_extreme_flatter_inventory",
            ["ipr_extreme_flatten"],
            flatten_soft=18,
            flatten_clip=4,
        ),
    ...
    (
        ...
        cfg(
            "R2-G2-04-IPR-SPREAD-RETUNE",
            "ipr_spread_retune",
            ["ipr_spread_retune"],
            ipr_continuous_spread=True,
            ipr_full_spread=4,
            ipr_mid_spread=14,
            ipr_max_spread=18,
            ipr_mid_mult=0.5,
            ipr_wide_mult=0.25,
            ipr_spread_penalty=0.08,
        ),
    ),
    (
        "candidate_r2_g2_05_aco_activation_probe.py",
        cfg("R2-G2-05-ACO-ACTIVATION-PROBE", "aco_activation_probe", ["aco_activation"], aco_probe_clip=4),
    ),
    (
        "candidate_r2_g2_06_aco_imbalance_wide_spread.py",
        cfg("R2-G2-06-ACO-IMBALANCE-WIDE-SPREAD", "aco_imbalance_wide_spread", ["aco_imbalance"], aco_clip=6),
    ),
    (
        "candidate_r2_g2_07_aco_reversal_wide_spread.py",
        cfg(
            "R2-G2-07-ACO-REVERSAL-WIDE-SPREAD",
            "aco_reversal_wide_spread",
            ["aco_reversal"],
            aco_clip=6,
            aco_reversal_threshold=0.5,
        ),
    ),
    (
        "candidate_r2_g2_08_combined_ipr_extreme_aco_probe.py",
        cfg(
            "R2-G2-08-COMBINED-IPR-EXTREME-ACO-PROBE",
            "combined_ipr_extreme_aco_probe",
            ["ipr_extreme", "aco_activation"],
            aco_probe_clip=3,
        ),
    ),
    (
        ...
        cfg(
            "R2-G2-09-COMBINED-IPR-EXTREME-ACO-IMBALANCE",
            "combined_ipr_extreme_aco_imbalance",
            ["ipr_extreme", "aco_imbalance"],
            aco_clip=5,
        ),
    ),
    (
        ...
        cfg(
            "R2-G2-10-SPREAD-OVERLAY-CONTINUOUS",
            "spread_overlay_continuous",
            ["ipr_spread_retune", "aco_imbalance"],
            ipr_continuous_spread=True,
            ipr_full_spread=4,
            ipr_mid_spread=14,
            ipr_max_spread=18,
    ...
    (
        ...
        cfg(
            "R2-G2-11-BRUNO-R1-KALMAN-R2-PORT",
            "bruno_r1_kalman_r2_port",
            ["ipr_extreme", "aco_kalman_r1"],
            aco_clip=10,
            aco_quote_clip=12,
            aco_kf_q=0.005,
            aco_kf_r=25.0,
    ...
    (
        ...
        cfg(
            "R2-G2-12-NOEL-R1-C26-R2-PORT",
            "noel_r1_c26_r2_port",
            ["ipr_extreme", "aco_noel_c26"],
            aco_clip=8,
            aco_quote_clip=12,
            aco_fixed_fv=10000.0,
            aco_take_edge=1,
        ),
    ),
    (
...
def render(config: dict) -> str:
    ...
    body = COMMON.replace("__CONFIG__", repr(config))
    return header + body


def main() -> None:
    OUT.mkdir(parents=True, exist_ok=True)
    for filename, config in CANDIDATES:
        (OUT / filename).write_text(render(config))
    readme_rows = [
        "# Bruno Round 2 Gen2 Canonical Bots",
        "",
        f"Generated from `{Path(__file__).relative_to(ROOT)}`.",
        f"Spec: `{SPEC}`.",
        "",
        "| File | Candidate | Role | Modes | MAF Bid |",
        "| --- | --- | --- | --- | ---: |",
    ]
    for filename, config in CANDIDATES:
        readme_rows.append(
            f"| `{filename}` | `{config['bot_id']}` | {config['role']} | `{','.join(config['modes'])}` | {config.get('maf_bid', 0)} |"
        )
    ...
    (OUT / "manifest_gen2.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    print(f"wrote {len(CANDIDATES)} bots to {OUT}")
