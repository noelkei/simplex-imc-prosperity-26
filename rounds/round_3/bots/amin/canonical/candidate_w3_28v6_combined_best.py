"""
W3-28v6: Combined Best — The "Everything" Variant
===================================================
Base: W3-28 (+1,168)
Combines the best ideas from v1-v5:
  From v1: HG_CAP=150, HG_PSZ=28, HG_TSZ=22 (more HYDRO capacity + aggression)
  From v2: EXP_GAMMA=4.0 (steeper penalty to prevent deep drawdowns)
  From v3: Phase 3 endgame unwind (protect 500-1000 from liquidation drag)
  From v4: VEX_CAP=80 (reduced VEX to limit adverse selection)
  From v5: HG_HALF=2 (tighter quotes for more fills when flat)

This is the "if everything that should help does help" variant.
Risk: combining many changes may interact negatively.
...
HG_CAP = 150; VEX_CAP = 80          # v1 HG up, v4 VEX down
HG_HALF = 2; VEX_HALF = 2           # v5 tighter HG
HG_PSZ = 28; VEX_PSZ = 10           # v1 HG up, v4 VEX down
HG_TSZ = 22; VEX_TSZ = 8            # v1 HG up, v4 VEX down
HG_TEDGE = 2; VEX_TEDGE = 2         # v1 tighter

EXP_GAMMA = 4.0                      # v2 steeper
IMB_GAIN = 2.5                       # v5 stronger lean
    ...
    def run(self, state: TradingState):
        ...
        if hg_od and hg_od.buy_orders and hg_od.sell_orders:
            ...
            if hg_ba - hg_bb >= 1:
                ...
                # Unwind at 50% of effective cap (v2-inspired earlier threshold)
                if hg_pos > eff_hg_cap * 0.5 and hgsr() > 0:
                    uq = min(25, hgsr())
                    hg_orders.append(Order(HG, hg_bb, -uq)); hg_s += uq
                elif hg_pos < -eff_hg_cap * 0.5 and hgbr() > 0:
                    uq = min(25, hgbr())
                ...
                if not no_new:
                    half = HG_HALF + (1 if abs(hg_pos) > eff_hg_cap * 0.4 else 0)
