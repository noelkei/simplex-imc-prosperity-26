...
r2_b04_kf_mle — R2 MLE Kalman parameters (reactive FV tracking)

HYPOTHESIS: R2 EDA maximum-likelihood calibration on 3 days (30k ticks) finds
Q=0.092, R=6.75, giving steady-state K≈0.11 (vs K≈0.014 from R1 c_07 with
Q=0.005, R=25). The MLE filter is 8× more reactive to mid-price movements.

When/why this helps:
  - If ACO FV experiences brief intraday drift events (price holds off 10000 for
    50-100 ticks), the reactive filter catches it sooner → better-centered quotes.
  - The R2 data still shows FV anchored near 10000 (cross-day drift ±1.4 ticks),
    so the extra reactivity mainly affects intraday tracking.

Risk: reactive filter partly tracks bid-ask bounce (ACF(lag1)=-0.50). The bounce
causes mid to oscillate ±3 ticks → FV oscillates ±0.33 ticks. Compared to
±0.04 ticks in R1 tuning. Both are small enough that risk is limited.

Unchanged: IMB_GAIN=2.0, IMB_CLIP=2.0, take threshold = raw fv_int.
Changed:   KF_Q=0.092, KF_R=6.75 (R2 MLE calibration).
"""
...
# R2 MLE calibration (Q_pooled=0.092, R_pooled=6.75)
KF_Q   = 0.092
KF_R   = 6.75
KF_P0  = 25.0
KF_FV0 = 10000.0
