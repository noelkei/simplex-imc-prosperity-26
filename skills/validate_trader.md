...
16. When the linked spec depends on a process hypothesis, multivariate relationship, cross-product relationship, or redundancy decision, state whether the run supports, weakens, contradicts, or does not test it. Use run diagnostics only as decision evidence, not as official Prosperity rules.
17. When platform `.json` or `.log` artifacts exist, fill the run summary `Run Classification`, `Feature Diagnostics`, `Process And Multivariate Diagnostics`, and `Post-Run Research` sections with failure-driven analysis, edge decomposition, lightweight counterfactuals, diagnostic method, knowledge delta, and the ROI-gated memory action.
18. Update or create `../rounds/round_X/workspace/post_run_research_memory.md` according to the ROI gate:
...
19. Record a provenance caveat if the exact `.py`, platform `.json`, and platform `.log` are not saved together. Missing one artifact does not block analysis, but the caveat must be visible.
20. For final upload candidates, run the pre-upload overfit / cheat audit from `../docs/templates/trader_production_template.md` and report `passed`, `failed`, or `caveat`.
21. Classify failures as rule or contract failure, implementation bug, data/EDA gap, heuristic weakness, execution tuning issue, evidence gap, or deadline tradeoff.
22. Do not treat round-local artifacts, `non-canonical/` drafts, sample PnL, uncalibrated replay, or post-run memory as official rules.
23. Determine the owning member from the bot path, round index, task request, or current phase owner. Use one of `isaac`, `bruno`, `amin`, `daniela`, or `noel`; ask only if ownership is unclear and matters for handoff.
24. Write current decision-supporting run summaries under `../rounds/round_X/performances/<member>/canonical/` using `run_YYYYMMDD_HHMM_<candidate_id>.md` or `run_YYYYMMDD_HHMM_<candidate_id>_<issue_id>.md`.
25. Include bot path, linked spec, insight being tested when applicable, linked signal/regime assumption, linked process hypothesis, linked multivariate relationship, linked redundancy decision, raw artifact path if available, run id, PnL source, diagnostics, errors/rejections/limit concerns, comparability, dedup verdict, knowledge delta, ROI-gated memory action, round adaptation audit outcome, contract readiness status, overfit/cheat audit outcome when applicable, post-run research memory file change, interpretation limits, signal/regime/process/multivariate evidence verdicts, decision, and next action.
26. Record whether the run is comparable to the baseline or prior run as `yes`, `no`, or `unclear`; non-comparable runs can support debugging but should not drive promotion without an explicit caveat.
27. State whether validation supports, weakens, contradicts, or does not test the linked signal/regime assumption, process hypothesis, multivariate relationship, or redundancy decision, with a brief basis.
28. Promote a candidate only when the evidence summary includes linked spec, bot path, contract/rule checks, one meaningful run or validation summary, concerns/caveats, PnL source, champion/challenger decision, and an explicit continue/promote/debug/discard/revise/rerun/stop decision.
29. Move or mark superseded, non-comparable, stale, or no-longer-decision-relevant summaries under `../rounds/round_X/performances/<member>/historical/`.
30. If valid behavior performs poorly, suggest returning to strategy/spec rather than treating the result as an implementation bug by default.
31. If validation contradicts the linked signal/regime assumption, process hypothesis, or multivariate relationship, route to strategy/spec or EDA before changing code unless there is also a rule or implementation failure.
32. Update `../rounds/round_X/workspace/_index.md` and `../rounds/round_X/workspace/phase_06_testing_context.md`, including latest validation run reference, PnL source, comparability, contract readiness status, ROI-gated memory action when decision-relevant, decision, and one-line note.
33. Report pass/fail, sources checked, linked spec/run if available, reproduction steps, caveats, PnL source, candidate class, memory action, and next action.
