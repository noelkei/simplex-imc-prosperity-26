...
| `r2_amin_timed_regime_07` | IPR + ACO | Phase-scheduled execution with regime-aware quoting | Fresh EDA suggests signal quality is not uniform; this branch tests whether changing risk posture over the session beats a stationary policy | Disruptive shift away from pure signal retuning toward execution scheduling | local harness does not clearly prefer it yet; time schedule may miss true live regime timing | medium | low | medium | medium | medium | medium | implemented |
| `r2_amin_regime_depth_tuned_08` | IPR + ACO | Fine-tuned successor to `_04` with harder ACO gating and slightly stronger IPR execution | Fresh re-check of Round 2 data shows ACO tight spreads are much more valuable than mid spreads; IPR remains broadly strong | Could improve PnL by reducing low-quality ACO exposure while monetizing IPR pressure more efficiently | local harness still prefers `_04`, so benefit may be subtle or data-split dependent | medium | medium | medium | medium | high | medium | implemented |
