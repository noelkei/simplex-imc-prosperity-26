"""
W3-28: Inventory-Optimal Delta1 + ITM
=======================================
W3-21 architecture with optimal inventory management:
  - Position sizes proportional to remaining capacity (Kelly-like)
  - Exponential inventory penalty (stronger at extremes than linear)
  - Inventory half-life targeting (positions decay toward zero over time)
  - Fills priority: when inventory high, only quote the reducing side

Key optimization: replace linear gamma * q/limit with exponential penalty
that becomes very steep near limits, preventing position blowups.
...
EXP_GAMMA = 3.0        # steepness of exponential penalty
...
def exp_penalty(pos, cap):
    """Exponential inventory penalty: 0 at pos=0, grows steeply near cap."""
    q = pos / max(1, cap)
    # Sign-preserving exponential: penalty grows much faster than linear near limits
    sign = 1.0 if q >= 0 else -1.0
    return sign * EXP_GAMMA * (math.exp(abs(q)) - 1.0) / (math.e - 1.0)


def prop_size(base, pos, cap, side):
    """Proportional sizing: scale down size as position approaches cap in that direction."""
    if side == "buy":
        room = max(0, cap - pos)
    else:
        room = max(0, cap + pos)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ══════════════════════════════════════════
        # HYDROGEL MM with exponential inventory
        # ══════════════════════════════════════════
        ...
        if hg_od and hg_od.buy_orders and hg_od.sell_orders:
            ...
            if hg_ba - hg_bb >= 1:
                ...
                # Aggressive unwind at 60% of cap
        ...
        # ══════════════════════════════════════════
        # VEX Kalman MM with exponential inventory
        # ══════════════════════════════════════════
