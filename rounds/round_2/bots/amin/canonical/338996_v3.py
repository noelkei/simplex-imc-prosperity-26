...
# ── Changes vs 338996_v2 (356073.py, score 9023) ────────────────────────────
# v3 Optiver-style improvements:
# 1. EMA-smoothed imbalance (α=0.4): reduces single-tick noise while preserving
#    the IC=0.647 lag-1 predictive power. Stored in traderData per tick.
# 2. Imbalance-scaled take sizing: 1.0–1.4× more units when the imbalance
#    signal aligns with the take direction (bullish imb → larger buy takes).
# 3. Aggressive unwind mode (|pos| >= 70): market-take orders force position
#    reduction in addition to skewed passive quotes. Inspired by ASML bot's
#    unwind mode that quotes aggressively at inventory extremes.
# 4. Dynamic take edge from realized volatility (10-tick rolling window):
#    wider edge in noisy markets (adverse-selection protection), tighter in
#    calm markets. Analogous to Optiver's dynamic credit = f(gamma, vega).
# 5. MAF bid: 1500 → 2000. Still EV-positive at 9023 base score (breakeven
#    ≈ 9023 × 1.25 / 0.8 − 9023 = 2256; bidding 2000 < breakeven).
# 6. Rolling mid history in traderData for realized-vol estimate.
# ─────────────────────────────────────────────────────────────────────────────
...
ACO_KALMAN_Q = 0.092          # MLE-calibrated
ACO_KALMAN_R = 6.75           # MLE-calibrated
ACO_MICROPRICE_WEIGHT = 0.4
ACO_MICROPRICE_CLIP = 2.0
ACO_DEPTH_SHIFT_CLIP = 2.0
ACO_IMBALANCE_GAIN = 5.0
ACO_IMBALANCE_SHIFT_CLIP = 5.0
ACO_IMBALANCE_EMA_ALPHA = 0.4  # EMA smoothing for imbalance signal
ACO_BASE_HALF_SPREAD = 2
ACO_TAKE_EDGE = 1.5
ACO_MAX_TAKE_ABOVE_FAIR = 0    # takes capped at Kalman fair; signal shifts quotes
ACO_TARGET_POSITION = 0
ACO_RESERVATION_SIGMA2 = 2.0
ACO_VOL_WINDOW = 10            # ticks for realized vol estimate
ACO_VOL_BASE = 2.8             # baseline σ of mid-price changes (ticks)
ACO_UNWIND_THRESHOLD = 70      # |pos| threshold for aggressive market unwind
ACO_UNWIND_QTY = 12            # max units forced per tick in unwind mode
MARKET_ACCESS_BID = 2000       # was 1500
...
class Trader:
    ...
    def _trade_ipr(self, state: TradingState, sd: dict):
        ...
        # Aggressive take: buy all available asks up to limit
    ...
    def _trade_aco(self, state: TradingState, sd: dict):
        ...
        # Rolling mid history for realized vol estimation
        mid_hist = aco_state.get("mid_hist", [])
        if obs is not None:
            mid_hist.append(obs)
            if len(mid_hist) > ACO_VOL_WINDOW:
                mid_hist = mid_hist[-ACO_VOL_WINDOW:]
        aco_state["mid_hist"] = mid_hist

        realized_vol = ACO_VOL_BASE
        if len(mid_hist) >= 4:
            changes = [mid_hist[i + 1] - mid_hist[i] for i in range(len(mid_hist) - 1)]
            mean_sq = sum(c * c for c in changes) / len(changes)
            realized_vol = math.sqrt(mean_sq) if mean_sq > 0 else ACO_VOL_BASE

        # EMA-smoothed imbalance: reduces single-tick noise in the IC=0.647 signal
        raw_imbalance = self._imbalance(od)
        prev_imb_ema = float(aco_state.get("imb_ema", 0.0))
        imb_ema = ACO_IMBALANCE_EMA_ALPHA * raw_imbalance + (1.0 - ACO_IMBALANCE_EMA_ALPHA) * prev_imb_ema
        aco_state["imb_ema"] = imb_ema
        ...
        # Predictive shift: use EMA imbalance for smoother quote positioning
        imbalance_shift = self._clip(
            ACO_IMBALANCE_GAIN * imb_ema,
        ...
        # Dynamic take edge: wider when realized vol is high (adverse-selection protection)
        vol_ratio = realized_vol / ACO_VOL_BASE
        vol_take_adj = self._clip((vol_ratio - 1.0) * 0.8, -0.5, 1.0)
        dynamic_take_edge = ACO_TAKE_EDGE + vol_take_adj + (1 if abs(pos) > 60 else 0)
        ...
        # Aggressive takes with imbalance-scaled sizing
        if od.sell_orders:
            for ask in sorted(od.sell_orders):
                ...
                base_take = 18 if edge < 2 else 28
                # Scale up when imbalance aligns (bullish → larger buy takes)
                imb_bull = max(0.0, imb_ema)
                take_qty = min(ask_qty, int(base_take * (1.0 + 0.35 * imb_bull)))
        ...
        if od.buy_orders:
            for bid in sorted(od.buy_orders, reverse=True):
                ...
                base_take = 18 if edge < 2 else 28
                # Scale up when imbalance aligns (bearish → larger sell takes)
                imb_bear = max(0.0, -imb_ema)
                take_qty = min(bid_qty, int(base_take * (1.0 + 0.35 * imb_bear)))
                if pos - self._sell_used < -55:
                    take_qty = min(take_qty, 10)
                self._add_sell(orders, ACO, bid, take_qty, limit)

        # Aggressive unwind mode: market-take to force position reduction at extremes.
        # Inspired by ASML bot's unwind mode. Fires only when |pos| >= 70.
        # Reduces position by up to ACO_UNWIND_QTY units per tick at market price.
        if pos >= ACO_UNWIND_THRESHOLD:
            unwind_qty = min(ACO_UNWIND_QTY, pos - 62)
            if unwind_qty > 0 and best_bid is not None and self._sell_cap(limit) > 0:
                self._add_sell(orders, ACO, best_bid, unwind_qty, limit)
        elif pos <= -ACO_UNWIND_THRESHOLD:
            unwind_qty = min(ACO_UNWIND_QTY, -pos - 62)
            if unwind_qty > 0 and best_ask is not None and self._buy_cap(limit) > 0:
                self._add_buy(orders, ACO, best_ask, unwind_qty, limit)
