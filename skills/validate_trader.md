...
15. For serious promotion decisions, extract practical run diagnostics when artifacts allow: product PnL split, final positions, own trades, buy/sell qty, matched qty, average buy/sell, gross spread capture, max drawdown, max abs position, and inventory/mark caveats. Use `pandas`/`polars` for logs, `numba` for slow repeated replay/markout loops, `pingouin`/`scipy`/`statsmodels` for confidence checks, and `ruptures`/`arch` for failure-window or volatility-regime diagnostics only when they could change promotion, backlog, or spec decisions. Mark unavailable fields as missing rather than blocking validation.
16. When platform `.json` or `.log` artifacts exist, fill the run summary `Run Classification` and `Post-Run Research` sections with failure-driven analysis, edge decomposition, lightweight counterfactuals, diagnostic method, knowledge delta, and the ROI-gated memory action.
