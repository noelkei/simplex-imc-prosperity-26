"""
W3-28v5: Tighter Spread + Larger Passive Sizes
================================================
Base: W3-28 (+1,168)
Changes:
  - HG_HALF: 3 → 2 (tighter passive quotes — inside the 15.7 spread more often)
  - HG_PSZ: 24 → 35 (much larger passive sizes to fill queue priority)
  - HG_TSZ: 18 → 25 (bigger aggressive takes)
  - VEX_PSZ: 15 → 20
  - Wider half-spread kicks in earlier: 40% of cap instead of 50%
  - Imbalance gain: 2.0 → 2.5 (stronger directional lean)

Rationale: HYDRO spread is 15.7 avg. Quoting at half=2 means 4-tick visible spread
which is still inside the market. Larger sizes get priority in fill queue. The
exponential penalty should handle the extra risk from tighter quotes.
This is the "be more aggressive about capturing spread" variant.
...
HG_HALF = 2; VEX_HALF = 1          # ← DOWN (tighter)
HG_PSZ = 35; VEX_PSZ = 20          # ← UP (larger)
HG_TSZ = 25; VEX_TSZ = 15          # ← UP (larger)
HG_TEDGE = 2; VEX_TEDGE = 1        # ← DOWN (tighter)

EXP_GAMMA = 3.0
IMB_GAIN = 2.5                      # ← UP from implicit 2.0
HALF_WIDEN_FRAC = 0.4               # ← DOWN from 0.5
    ...
    def run(self, state: TradingState):
        ...
        if hg_od and hg_od.buy_orders and hg_od.sell_orders:
            ...
            if hg_ba - hg_bb >= 1:
                ...
                sd["hi"] = hg_imb
                hg_qfair = hg_micro - exp_penalty(hg_pos, HG_CAP) + clamp(IMB_GAIN * hg_imb, -3, 3)
                ...
                half = HG_HALF + (1 if abs(hg_pos) > HG_CAP * HALF_WIDEN_FRAC else 0)
