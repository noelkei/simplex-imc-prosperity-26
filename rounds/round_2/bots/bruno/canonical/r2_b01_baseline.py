...
r2_b01_baseline — Control: R1 c_07 logic + MAF bid

HYPOTHESIS: Establish the P&L baseline. Every other R2 bot is measured against this.

Base: candidate_07_kf_tuned (Round 1 best — R1-calibrated Kalman, relaxed take filter).
Changes: adds bid() for MAF=2500.

ACO parameters:
  KF_Q=0.005, KF_R=25.0 → K_ss≈0.014 (very stable FV near 10000)
  IMB_GAIN=2.0, IMB_CLIP=2.0 (original imbalance micro-shift from c_26_v3)
  Take threshold: raw KF FV (fv_int) — does NOT use qfair for takes
  Quote sizes: 72/60/40 by position only

Expected P&L: ~9,200-9,400 per backtest run (IPR≈7,286, ACO≈2,000-2,100).
"""
...
# Kalman filter — R1 c_07 calibration (very stable)
KF_Q   = 0.005
KF_R   = 25.0
KF_P0  = 25.0
KF_FV0 = 10000.0
