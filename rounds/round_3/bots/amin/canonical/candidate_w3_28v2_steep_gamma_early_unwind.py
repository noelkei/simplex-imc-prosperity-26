"""
W3-28v2: Steeper Gamma + Earlier Unwind
=========================================
Base: W3-28 (+1,168)
Changes:
  - EXP_GAMMA: 3.0 → 4.5 (much steeper penalty near limits — prevents deep drawdowns)
  - Unwind threshold: 60% → 45% (start unwinding earlier to avoid the -750 drawdowns)
  - Unwind qty: 20 → 25 (more aggressive when triggered)
  - HG_CAP: 130 → 110 (slightly tighter to prevent extreme positions)

Rationale: The PnL curve shows -750 drawdown early and 1K+ giveback late.
Steeper gamma + earlier unwind should reduce both by preventing inventory extremes.
Trade-off: may capture fewer fills, but protects more PnL.
...
HG_CAP = 110; VEX_CAP = 100       # ← DOWN from 130/120
HG_HALF = 3; VEX_HALF = 2
HG_PSZ = 24; VEX_PSZ = 15
HG_TSZ = 18; VEX_TSZ = 12
HG_TEDGE = 3; VEX_TEDGE = 2

EXP_GAMMA = 4.5                    # ← UP from 3.0
UNWIND_FRAC = 0.45                  # ← DOWN from 0.6
UNWIND_QTY = 25                     # ← UP from 20
    ...
    def run(self, state: TradingState):
        ...
        if hg_od and hg_od.buy_orders and hg_od.sell_orders:
            ...
            if hg_ba - hg_bb >= 1:
                ...
                if hg_pos > HG_CAP * UNWIND_FRAC and hgsr() > 0:
                    uq = min(UNWIND_QTY, hgsr())
                    hg_orders.append(Order(HG, hg_bb, -uq)); hg_s += uq
                elif hg_pos < -HG_CAP * UNWIND_FRAC and hgbr() > 0:
                    uq = min(UNWIND_QTY, hgbr())
