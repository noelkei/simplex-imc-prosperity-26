...
r2_b02_imb4 — Stronger imbalance signal (gain=4)

HYPOTHESIS: R2 EDA finds imbalance IC=0.647 (n=30k). The baseline uses gain=2.0
which captures only 29% of the signal magnitude. Doubling to gain=4.0 shifts
quotes by ±4 ticks at full imbalance, better aligning quotes with expected
next-tick mid movement (predicted β=6.85 ticks/unit imbalance).

Key effects:
  - When imb=1 (bid-heavy, price rising): ask shifts +4 ticks → we sell 4 ticks higher
  - When imb=-1 (ask-heavy, price falling): bid shifts -4 ticks → we buy 4 ticks cheaper
  - Both reduce adverse selection and improve fill prices

Unchanged: KF_Q=0.005, KF_R=25.0 (stable R1 Kalman), take threshold (raw fv_int).
Changed:    IMB_GAIN=4.0, IMB_CLIP=4.0 (vs 2.0/2.0 in baseline).
"""
...
ACO_HS   = 5
ACO_SKEW = 3
IMB_GAIN = 4.0   # increased from 2.0
IMB_CLIP = 4.0   # increased from 2.0
