...
class Trader:
    ...
    def _trade_aco(self, state: TradingState, sd: dict):
        ...
        signal_dir = 1 if predictive_shift > 0 else -1 if predictive_shift < 0 else 0
        conviction = abs(predictive_shift)
        if spread is not None and spread <= 14:
            conviction += 0.25
        elif spread is not None and spread >= 18:
            conviction += 0.1

        sprint = sd.get("aco_sprint", {})
        prev_dir = int(sprint.get("dir", 0))
        prev_age = int(sprint.get("age", 0))
        prev_strength = float(sprint.get("strength", 0.0))
        persistent = signal_dir != 0 and signal_dir == prev_dir and conviction >= max(ACO_HIGH_CONVICTION, prev_strength - 0.2)
        start_sprint = signal_dir != 0 and conviction >= ACO_SPRINT_ENTRY and persistent and abs(pos) <= 24
        keep_sprint = signal_dir != 0 and signal_dir == prev_dir and conviction >= ACO_SPRINT_DECAY and prev_age < ACO_SPRINT_MAX_HOLD
        ...
        if start_sprint:
            sprint_mode = True
            sprint_dir = signal_dir
            sprint_age = 1
        elif keep_sprint:
            sprint_mode = True
            sprint_dir = prev_dir
            sprint_age = prev_age + 1

        sd["aco_sprint"] = {
            "dir": sprint_dir if sprint_mode else signal_dir,
            "age": sprint_age if sprint_mode else 0,
            "strength": conviction,
        }
        ...
        if conviction < ACO_MEDIUM_CONVICTION:
            dynamic_take_edge += 0.4
        if od.sell_orders:
            for ask in sorted(od.sell_orders):
                if self._buy_cap(limit) <= 0:
                    break
                edge = quote_fair - ask
                if sprint_mode and sprint_dir > 0:
                    edge += 0.45
                if edge < dynamic_take_edge:
                    break
                ask_qty = -od.sell_orders[ask]
                take_qty = min(ask_qty, 18 if edge < dynamic_take_edge + 1 else 28)
                if conviction >= ACO_HIGH_CONVICTION:
                    take_qty = min(ask_qty, max(take_qty, 28))
                if sprint_mode and sprint_dir > 0:
                    take_qty = min(ask_qty, max(take_qty, 34))
                if pos + self._buy_used > 55:
                    take_qty = min(take_qty, 10 if not sprint_mode else 16)
        ...
        if od.buy_orders:
            for bid in sorted(od.buy_orders, reverse=True):
                ...
                if conviction >= ACO_HIGH_CONVICTION:
                    take_qty = min(bid_qty, max(take_qty, 28))
                if sprint_mode and sprint_dir < 0:
                    take_qty = min(bid_qty, max(take_qty, 34))
        ...
        if pos < -20:
            ...
            sell_size = min(sell_size, 18 if not sprint_mode else sell_size)
        elif pos > 20:
            buy_size = min(buy_size, 18 if not sprint_mode else buy_size)
            sell_size = max(sell_size, 56)

        if abs(pos) > 60:
            if pos > 0:
                buy_size = 8 if not sprint_mode else min(buy_size, 24)
                sell_size = 64
            else:
                buy_size = 64
                sell_size = 8 if not sprint_mode else min(sell_size, 24)

        if sprint_mode and sprint_dir > 0:
            if self._buy_cap(limit) > 0 and (best_ask is None or bid_px < best_ask):
                self._add_buy(orders, ACO, bid_px, buy_size, limit)
            if pos + self._buy_used > 25 and self._sell_cap(limit) > 0 and (best_bid is None or ask_px > best_bid):
                self._add_sell(orders, ACO, ask_px, sell_size, limit)
        elif sprint_mode and sprint_dir < 0:
            if self._sell_cap(limit) > 0 and (best_bid is None or ask_px > best_bid):
                self._add_sell(orders, ACO, ask_px, sell_size, limit)
            if pos - self._sell_used < -25 and self._buy_cap(limit) > 0 and (best_ask is None or bid_px < best_ask):
                self._add_buy(orders, ACO, bid_px, buy_size, limit)
        else:
            if self._buy_cap(limit) > 0 and (best_ask is None or bid_px < best_ask):
                self._add_buy(orders, ACO, bid_px, buy_size, limit)
