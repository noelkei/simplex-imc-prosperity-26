...
class Trader:
    ...
    def _trade_ipr(self, state: TradingState, sd: dict):
        ...
        phase = 0 if state.timestamp < 25000 else 1 if state.timestamp < 70000 else 2
        aggressive_limit = 76 if phase == 0 else 72 if phase == 1 else 64
    ...
    def _trade_aco(self, state: TradingState, sd: dict):
        ...
        if phase == 0:
            dynamic_take_edge -= 0.2
        elif phase == 2:
            dynamic_take_edge += 0.6
        if od.sell_orders:
            for ask in sorted(od.sell_orders):
                if self._buy_cap(limit) <= 0:
                    break
        ...
        buy_size = 44 if phase == 0 else 40 if phase == 1 else 24
        sell_size = 44 if phase == 0 else 40 if phase == 1 else 24
