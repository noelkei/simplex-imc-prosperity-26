...
IPR_REPOST_MAX_EXTRA = 3
IPR_PRESSURE_REPOST_COEF = 4.0
IPR_STRONG_ENTRY = 1.4
IPR_MEDIUM_ENTRY = 0.8
MARKET_ACCESS_BID = 9
...
class Trader:
    ...
    def _trade_ipr(self, state: TradingState, sd: dict):
        ...
        signal = 0.95 * imbalance + 0.55 * depth_shift
        if spread is not None and spread >= 15:
            signal += 0.4 * depth_shift
        elif spread is not None and spread <= 13:
            signal += 0.2 * imbalance

        aggressive_limit = 76 if signal >= IPR_STRONG_ENTRY else 68 if signal >= IPR_MEDIUM_ENTRY else 56
        max_take = 18 if signal >= IPR_STRONG_ENTRY else 10 if signal >= IPR_MEDIUM_ENTRY else 4
        ...
        for ask in sorted(od.sell_orders):
            ...
            edge_ok = visible_mid is None or ask <= visible_mid + (1 if signal >= IPR_MEDIUM_ENTRY else 0)
            if not edge_ok and signal < IPR_STRONG_ENTRY:
                continue
            take_qty = min(ask_qty, max_take)
            if target_pos >= aggressive_limit:
                take_qty = min(take_qty, 4 if ask >= (visible_mid if visible_mid is not None else ask) else 6)
            self._add_buy(orders, IPR, ask, take_qty, limit)

        if best_bid is not None and self._buy_cap(limit) > 0 and signal >= 0.15:
            pressure_score = max(0.0, signal)
            ...
            if signal < IPR_MEDIUM_ENTRY:
                desired_repost = min(desired_repost, 12)
            elif signal < IPR_STRONG_ENTRY:
                desired_repost = min(desired_repost, 24)
            self._add_buy(orders, IPR, bid_px, desired_repost, limit)

        effective_pos = pos + self._buy_used - self._sell_used
        if effective_pos > 64 and best_bid is not None:
            trim_trigger = (visible_mid if visible_mid is not None else best_bid) + (IPR_RICH_TRIM_EDGE - (2 if signal < 0.3 else 0))
            if best_bid >= trim_trigger:
                self._add_sell(orders, IPR, best_bid, min(10, effective_pos - 64), limit)
    ...
    def _trade_aco(self, state: TradingState, sd: dict):
        ...
        active_regime = spread is not None and (spread <= 14 or spread >= 18)
        if active_regime:
            predictive_shift = 0.48 * micro_shift + 0.24 * depth_shift + imbalance_shift
        else:
            predictive_shift = 0.3 * micro_shift + 0.04 * depth_shift + 0.8 * imbalance_shift
        ...
        buy_size = 38 if active_regime else 30
        sell_size = 38 if active_regime else 30
